...
sub pasti(){
   ...
   print 'Indice '.$i.' pagina '.$pag.' i<'.(10*($pag+1)).'?';
   if($i>9 and $i>9){
          $pag++;
...
sub elencoRicette(){
	print '<div class="lista">
				...
				<div class="descr">
					<p>Nome: <a class="titolo" href="visualizzaRicetta.pl?nome='.$_[0]->find('./nome').'&tipo='.$_[0]->find('./tipo').'" class="nomeRicetta">'.$_[0]->find('./nome').'</a></p>
					<p class="autore">Autore: '.$_[0]->find('./autore').'</p>';
					my $procedimento=substr($_[0]->find('./procedimento'),0,60);
					print '<div><p class="procedimento">Procedimento: '.$procedimento.'... <a href="visualizzaRicetta.pl?nome='.$_[0]->find('./nome').'&tipo='.$_[0]->find('./tipo').'">Leggi tutto</a></p></div>
				</div>
			 </div>';
}
